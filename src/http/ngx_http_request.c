...
static int
ngx_ssl_empty_password_callback(char *buf, int size, int rwflag, void *userdata)
{
	*buf = 0;
	return 0;
}
...
ngx_http_ssl_servername(ngx_ssl_conn_t *ssl_conn, int *ad, void *arg)
{
    ...
    if (ngx_http_validate_host(&host, c->pool, 1) != NGX_OK) {
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "ngx_http_ssl_servername ngx_http_validate_host != ok");
		return SSL_TLSEXT_ERR_NOACK;
    }
    ...
    {
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "ngx_http_ssl_servername ngx_http_find_virtual_server != ok");
		//return SSL_TLSEXT_ERR_NOACK;
	}
	else {
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "ngx_http_ssl_servername ngx_http_find_virtual_server == ok");
    ...
    if (hc->ssl_servername == NULL) {
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "ngx_http_ssl_servername hc->ssl_servername == NULL");
		return SSL_TLSEXT_ERR_NOACK;
    }
    ...
    *hc->ssl_servername = host;
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "Autogen: %d addr_text=%V", sscf->autogen, &c->addr_text);
	...
	if (sscf->ssl.ctx) {
		...
		if (sscf->autogen) {
			...
			int portNumber = 443;
			if (portNumber != 443) {
				snprintf(hostAndPort, sizeof(hostAndPort), "%s:%d", servername, portNumber);
			}
			else {
				snprintf(hostAndPort, sizeof(hostAndPort), "%s", servername);
			}
			char *certPath, *certKey;
			if (!ngx_get_cert(c->log, c->pool, sscf, hostAndPort, &certPath, &certKey,
				SSL_CTX_get0_certificate(sscf->ssl.ctx), SSL_CTX_get0_privatekey(sscf->ssl.ctx))) { 
				return SSL_TLSEXT_ERR_NOACK;
			}

			SSL_set_default_passwd_cb(ssl_conn, ngx_ssl_empty_password_callback);
			SSL_use_certificate_file(ssl_conn, certPath, SSL_FILETYPE_PEM);
			SSL_use_PrivateKey_file(ssl_conn, certKey, SSL_FILETYPE_PEM);

			ngx_pfree(c->pool, certPath);
			ngx_pfree(c->pool, certKey);
		}
		else {
